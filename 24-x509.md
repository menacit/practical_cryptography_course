...
Get ready for some new exciting acronyms!

![bg right:30%](images/24-red_eft.jpg)
...
certificates based on CSRs after some validation.

_(is the person requesting a certificate for
"bob@example.com" really the rightful
"owner" of "bob@example.com"?)_
...
## Intermediate CA
CA whose certificate is signed by a root CA,
commonly used to sign day-to-day CSRs.

_(may be rotated without updating truststores)_
...
A root CA is included in truststores.
It's typically used to sign an
intermediate CA certificate.

The intermediate CA validates CSRs
and issues client/server certificates.

CRL and OCSP is used to check if a
valid certificate should still be trusted.

![bg right:30%](images/24-ake_head_statues_and_berries.jpg)
...
```
$ cfssl gencert -initca ca-csr_template.json | cfssljson -bare ca
...
$ cfssl gencert \
	  ...
	  my_server-csr_template.json \
  | cfssljson -bare my_server
