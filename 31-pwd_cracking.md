...
Keys for symmetric encryption are
typically generated by a KDF
(basically a fancy hash function\*).  
  
Asymmetric private keys are often
stored using symmetric cryptography.
...
```
$<hash_type>$<options>$<password_hash>
```

("options" could be salt, rounds, etc.)
...
- "**2a**": Blowfish
- "**2y**": Eksblowfish
- "**5**": SHA-256 crypt (SHA-2)
- "**6**": SHA-512 crypt (SHA-2)
- "**y**": yescrypt
....
...
Credentials for local and "cached" users
are stored in the "**SAM database**".  
  
Also loves storing credentials for
authenticated users in memory.
...
Disabled by default in Windows Vista (2007),
...
<!-- _footer: "%ATTRIBUTION_PREFIX% Adam Lusch (CC BY-SA 2.0)" -->
## LM hash
1. Convert lower case to upper case characters
2. "Pad" password to 14 characters with NULL characters, if needed - truncate otherwise
3. Split the password to two 7 character chunks
4. Create two DES keys from each chunk
5. DES (ECB) encrypt the string "KGS!@#$%" with each chuck as the key
6. Concatenate the two DES encrypted strings.

![bg right:30%](images/31-bolt_and_nut.jpg)

---
<!-- _footer: "%ATTRIBUTION_PREFIX% Adam Lusch (CC BY-SA 2.0)" -->
Why is usage of "LM hash" far
from ideal for password storage? 

![bg right:30%](images/31-bolt_and_nut.jpg)

---
<!-- _footer: "%ATTRIBUTION_PREFIX% Michał Bulsa (CC BY-SA 4.0)" -->
## NTLM/NTHash
...
![bg right:30%](images/31-szopienice_water_tower.jpg)
...
- [**John The Ripper**](https://www.openwall.com/john/): Swiss army knife, also known as "john" and "jtr"
- [**Hashcat**](https://hashcat.net/): When you wanna crack 'em fast, primarily works on GPUs!
...
Well, let's run a test with a [RTX 4090](https://www.tradera.com/search?q=RTX%204090):
...
Bitcoin uses SHA-2 256.  
  
RTX 4090 \* 5 can do \~110 GH/s.  
S21 Hyd. can do **\~355000 GH/s**.  
  
PoW != password cracking,
but gives us an idea what you
could do with custom hardware!
